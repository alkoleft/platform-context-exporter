...
# Форматы и примеры использования

## Улучшенные описания параметров MCP инструментов

### Проблема
LLM часто создает неточные запросы при поиске в API 1С. Например, при запросе "получение параметров запроса" LLM может создать запрос "параметры запроса", что не дает результатов.

### Решение
Добавлены подробные описания параметров с примерами и автоматическое преобразование пользовательских запросов в более точные поисковые термины.

### Примеры использования

#### Инструмент `search`
**Описание**: Поиск по API платформы 1С Предприятие. Используйте конкретные термины 1С для получения точных результатов.

**Параметры**:
- `query` - Поисковый запрос с примерами:
  ...
  - Примеры хороших запросов: "методы работы со справочниками", "получение данных из базы"
...
#### Инструмент `info`
**Описание**: Получение детальной информации об элементе API платформы 1С. Требует точное имя элемента.

**Параметры**:
- `name` - Точное имя элемента API с примерами:
  - Для методов: "НайтиПоСсылке", "ВыполнитьОбработку"
  - Для типов: "СправочникСсылка", "ДокументОбъект"
  - Для свойств: "Ссылка", "Код", "Наименование"

#### Инструмент `getMember`
**Описание**: Получение информации о методе или свойстве конкретного типа 1С.

**Параметры**:
- `typeName` - Имя типа 1С:
...
- `memberName` - Имя метода или свойства:
  - Для справочников: "НайтиПоКоду", "НайтиПоНаименованию", "Код"
  - Для документов: "Записать", "Провести", "Дата", "Номер"
  - Для строк: "Длина", "ВРег", "НРег", "СокрЛП"

### Рекомендации по использованию

1. **Используйте конкретные термины 1С** вместо общих описаний
2. **Указывайте тип поиска** для более точных результатов
3. **Следите за регистром** - имена в 1С чувствительны к регистру
4. **Используйте примеры** из документации для формулировки запросов

### Преимущества улучшенных описаний

- **Повышение точности**: LLM лучше понимает, что искать
- **Уменьшение ложных результатов**: Более конкретные запросы дают релевантные результаты  
- **Улучшение пользовательского опыта**: Пользователи получают нужную информацию быстрее
- **Автоматическая коррекция**: Система сама исправляет неточные запросы